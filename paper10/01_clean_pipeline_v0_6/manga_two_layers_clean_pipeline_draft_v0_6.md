...
**Draft status:** v0.6 public-facing technical draft with research passes integrated and internal scaffolding removed  
**Prepared for:** Team Hermes voice pass and Zenodo packaging  
...
MaNGA DynPop cannot be used as a strict board for the locked Hermes rotation-curve equation, because its circular velocities and masses are inferred from stellar-dynamical JAM models rather than observed H I/Hα rotation curves with independent gas, disk, and bulge components. It can, however, test a weaker principle-level question: whether galaxy age organizes dynamical mass-to-light structure after stellar mass and standard structural variables are held fixed.

We joined the public MaNGA DynPop JAM, stellar-population/star-formation-history, and circular-velocity-curve catalogs. The matched catalog contains 10,296 galaxies; after the conservative `Qual >= 1` cut and finite-value requirements, the primary decision sample contains 5,952 galaxies. We split this sample into eight equal-count stellar-mass bins and compared the youngest 25% and oldest 25% by `T50` within each bin.

The blunt mass-to-light discrepancy test used \(D_{ML}=\log_{10}(M/L)_{dyn}-\log_{10}(M_*/L)_{SPS}\). Young galaxies show larger observed-SPS DML than old galaxies in 8/8 stellar-mass bins and larger intrinsic-SPS DML in 6/8 bins. A hardening test shows that this raw result is mostly carried by the SPS denominator: older stellar populations have higher SPS stellar mass-to-light ratios, as standard stellar-population synthesis predicts. Raw JAM dynamical mass-to-light alone goes the opposite way, with old galaxies higher in all eight bins.

The candidate result is therefore not the raw DML split. It is the smaller controlled dynamical mass-to-light residual that remains after SPS mass-to-light, metallicity, stellar mass, size, surface density, Sérsic structure, flattening, \(\lambda_R\), and velocity dispersion are included as controls. In that residual split, young galaxies remain higher in 8/8 mass bins for the observed-SPS and both-SPS control sets, and in 6/8 bins for the intrinsic-SPS control set.

We interpret this as a two-layer age structure in MaNGA DynPop: a dominant, expected stellar-population mass-to-light layer and a smaller controlled dynamical mass-to-light residual layer. The first layer is standard astrophysics. The second is not yet anomalous; existing JAM/SPS, IMF, structural-covariance, assembly-history, and simulation pathways could plausibly contribute. A targeted simulation pass found adjacent MaNGA-like mock infrastructure, but not an exact mock-MaNGA/JAM/SPS replication of this residual pipeline. The note therefore makes a limited proof-of-concept claim: the public DynPop catalogs contain a reproducible two-layer age pattern, and the conventional separation between the visible SPS layer and the controlled dynamical residual is a testable filing-system question, not a settled ontological boundary.
...
6. test whether a smaller JAM dynamical age residual remains after controlling for SPS mass-to-light, metallicity, stellar mass, size, surface density, Sérsic structure, flattening, \(\lambda_R\), and velocity dispersion.
...
The main age variable is `T50`. Larger `T50` means older assembly.
...
The blunt DML split is visually and numerically strong.

| Outcome | Young > old bins | Mean bin young-old median difference | Mass-adjusted young-old difference | Bootstrap 95% CI | Joint shuffle p |
|---|---:|---:|---:|---:|---:|
| `DML_mfl_int` | 6 / 8 | +0.0606 | +0.0772 | [+0.0562, +0.0959] | 0.0002 |
| `DML_mfl_obs` | 8 / 8 | +0.0742 | +0.0669 | [+0.0498, +0.0828] | 0.0002 |

The observed-SPS DML result is the cleaner visual result: young galaxies have larger DML than old galaxies in all eight mass bins. The intrinsic-SPS result is positive overall but has two low-mass bins with young below old.
...
| Controlled residual outcome | Young > old bins | Mean young-old residual | Mass-adjusted young-old residual | Bootstrap 95% CI | Joint shuffle p |
|---|---:|---:|---:|---:|---:|
| After intrinsic SPS controls | 6 / 8 | +0.0210 | +0.0188 | [+0.0092, +0.0312] | 0.0002 |
| After observed SPS controls | 8 / 8 | +0.0359 | +0.0358 | [+0.0270, +0.0475] | 0.0002 |
| After both SPS controls | 8 / 8 | +0.0342 | +0.0311 | [+0.0216, +0.0436] | 0.0002 |
...
The cleanest interpretation is that MaNGA shows a two-layer age structure and gives us a reason to test whether the usual two-box filing system is complete.
...
The cautious form is:

> The standard account explains the dominant stellar-population layer. The present pipeline finds a smaller dynamical residual after that layer is stripped away. The residual is directionally consistent with the Hermes/Emergent Plane age prediction, but it requires specialist hardening before interpretation.

The stronger discussion form is:

> The standard explanation may be locally correct but globally incomplete.

That is the posture of this note. It does not fight the standard explanation for Layer 1. It grants it. Then it asks whether Layer 2 should be filed as unrelated noise, IMF/SPS covariance, assembly history, or as a second age-organized layer that deserves joint interpretation.
...
### 10.3 IMF variation remains open
...
Both must remain in the report.
...
### 10.7 fDM does not confirm the signal

The NFW and gNFW dark-matter fractions within \(R_e\) do not show the same age signal. That does not automatically refute the DML residual, but it blocks any simple claim that MaNGA directly shows age-dependent dark matter fraction.
...
The analysis measures a two-layer pattern. It does not measure a common mechanism behind the two layers. The one-box interpretation is a way to organize the question, not an answer to it. A standard-model account could still explain the residual through IMF variation, assembly history, structural covariance, SPS mismatch, dust treatment, JAM assumptions, or simulation-predicted age correlations. The paper should invite those tests rather than preempt them.
...
The public note should eventually include a compact reproduction script, a manifest with SHA256 hashes, and a clear description of which files are raw public inputs versus derived outputs. That packaging is not complete in this Step 1 draft.
...
## 14. Tables for the main text

### Table 1. Primary sledgehammer scoreboard

| Outcome | Young > old bins | Mean young-old median difference | Mass-adjusted young-old difference | Bootstrap 95% CI | Joint shuffle p |
|---|---:|---:|---:|---:|---:|
| `DML_mfl_int` | 6 / 8 | +0.0606 | +0.0772 | [+0.0562, +0.0959] | 0.0002 |
| `DML_mfl_obs` | 8 / 8 | +0.0742 | +0.0669 | [+0.0498, +0.0828] | 0.0002 |
...
When the data are split bluntly by age at fixed stellar mass, young galaxies show larger DML than old galaxies. That result is strong by the simple scoreboard: 8/8 mass bins for observed SPS and 6/8 for intrinsic SPS, with shuffle p-values at the 5,000-trial floor.
...
The research-front passes are now folded into the draft: the novelty check appears in Section 3, the SPS/IMF/JAM-SPS literature pass appears in Sections 3.2, 3.3, 9, 10, and 15, the one-box interpretive pass appears in Sections 1, 9, 10, 11, and 15, and the simulation/assembly-bias pass appears in Sections 3.4, 10.8, 15, and 16.2. One quantitative SPS check remains useful before Zenodo release; the true standard-model mock comparator remains a future-work item rather than a prerelease requirement.

### 16.1 Quantitative SPS expectation and dust convention check

The medium-depth literature pass establishes the qualitative point: old stellar populations should have higher SPS \(M_*/L\), and JAM/SPS offsets are already used as IMF/dark-matter diagnostics. A final public note would benefit from one quantitative addition:
...
### 16.3 Recommended wording

Unless the remaining literature pass finds an exact prior analysis, the public wording should be:
...
## 17. Working references added through v0.6
